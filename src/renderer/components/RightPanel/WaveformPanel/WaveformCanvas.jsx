...
import { barDurationMs } from '../../../utils/cueCalc.js'

// Resolve one display pixel → { max, min, rms } from the peaks array.
// Handles both downsampling (many peaks → 1 px) and upsampling (1 peak → many px).
function resolvePx(slice, x, W) {
  ...
  if (i1 - i0 <= 1) {
    // Upsampling — linear interpolate between adjacent peaks
    const frac = f0 - i0
  ...
  // Downsampling — true max/min/rms across all contributing peaks
...
export default function WaveformCanvas({
  ...
  propsRef.current = { startMs, endMs, zoom, scrollMs, track, onScroll, onZoom }
  ...
  useEffect(() => {
    ...
    if (loading) {
      ...
      for (let x = 0; x < waveW; x++) {
        ...
        // Outer peak envelope (symmetric)
        ctx.fillStyle = 'rgba(8,90,210,0.32)'
        ctx.fillRect(x, mid - oh, 1, oh * 2)

        // RMS core (symmetric)
        ctx.fillStyle = 'rgba(30,140,255,0.88)'
        ctx.fillRect(x, mid - rh, 1, rh * 2)

        // Bright spine (symmetric)
        ctx.fillStyle = 'rgba(120,190,255,0.55)'
        ctx.fillRect(x, mid - sh, 1, sh * 2)
      }
    } else {
    ...
    if (track.beats && track.beats.length > 0) {
      // Use exact ANLZ beat positions — identical to rekordbox, no drift
      ...
      // Fallback: compute from BPM when no ANLZ data
  ...
  useEffect(() => {
    ...
    const onMouseDown = (e) => {
      e.preventDefault()
      dragRef.current = { startX: e.clientX, startScrollMs: propsRef.current.scrollMs }
    ...
    const onMouseMove = (e) => {
      ...
      onScroll?.(Math.max(0, Math.min(
        dragRef.current.startScrollMs + deltaMs,
        durationMs - spanMs
      )))
    }
    ...
    const onWheel = (e) => {
      ...
      // Smooth exponential zoom: each scroll unit = ~1% zoom change
      const newZoom    = Math.max(1, Math.min(64, zoom * Math.exp(-e.deltaY * 0.002)))
      const newVisible = durationMs / newZoom
      onZoom?.(newZoom, Math.max(0, Math.min(
        cursorMs - cursorFrac * newVisible,
        durationMs - newVisible
      )))
    }
